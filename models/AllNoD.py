...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.KAN_channel_freq = self._init_kan_layer(KAN.KAN, [self.feature_size // 2 + 1, self.hidden_size,
                                                                  self.feature_size // 2 + 1])
        self.KAN_channel_time = self._init_kan_layer(KAN.KAN, [self.seq_length // 2 + 1, self.hidden_size,
                                                               self.seq_length // 2 + 1])
        ...
        self.fc = self._init_kan_layer(KAN.KAN, [self.seq_length, self.hidden_size, self.pre_length])
    ...
    def KAN_temporal(self, x, B, N, L):
        ...
        if self.use_comKAN == 1:
            y_real = self.KAN_channel_time(x.real)
            y_imag = self.KAN_channel_time(x.imag)
    ...
    def KAN_channel(self, x, B, N, L):
        ...
        y_real = self.KAN_channel_freq(x.real)
        y_imag = self.KAN_channel_freq(x.imag)
        # y = torch.complex(y_real, y_imag)
    ...
    def forward(self, x):
        ...
        if self.channel_independence == 1:
            x = self.KAN_channel(x, B, N, T)
            x_noD = self.KAN_channel_WO_freq(x, B, N, T)
        # [B, N, T, D]
        elif self.channel_independence == 0:
            x = self. KAN_temporal(x, B, N, T)
            x_noD = self. KAN_temporal_WO_freq(x, B, N, T)
        else:
            x = self. KAN_channel(x, B, N, T)
            x = self. KAN_temporal(x, B, N, T)
            x_noD = self. KAN_channel_WO_freq(x, B, N, T)
            x_noD = self. KAN_temporal_WO_freq(x_noD, B, N, T)
        x = x + x_noD
        if self.use_bias:
            x = x + bias
        else:
