...
class KANLinear(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        B, nd, dimension, _ = x.shape

        x_real = x.real.view(B * nd, dimension, self.in_features)
        x_imag = x.imag.view(B * nd, dimension, self.in_features)

        x_real = x_real.reshape(-1, self.in_features)
        x_imag = x_imag.reshape(-1, self.in_features)

        base_output_real = F.linear(self.base_activation(x_real), self.base_weight_real) \
                           - F.linear(self.base_activation(x_imag), self.base_weight_imag)
        ...
        # output_real = output_real.reshape(*original_shape[:-1], self.out_features)

        base_output_imag = F.linear(self.base_activation(x_imag), self.base_weight_real) \
                           + F.linear(self.base_activation(x_real), self.base_weight_imag)
        ...
        # output_imag = output_imag.reshape(*original_shape[:-1], self.out_features)
