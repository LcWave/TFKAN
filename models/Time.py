...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.MLP_in = nn.Linear(self.embed_size, self.hidden_size)
        self.relu = nn.ReLU()
        self.MLP_out = nn.Linear(self.hidden_size, self.embed_size)
    ...
    def NorMLP_temporal(self, x, B, N, L):
        # [B, N, T, D]
        x = torch.fft.rfft(x, dim=2, norm='ortho')  # FFT on L dimension
        y_real = self.MLP_in(x.real)
        y_real = self.relu(y_real)
        y_real = self.MLP_out(y_real)
        y_imag = self.MLP_in(x.imag)
        y_imag = self.relu(y_imag)
        y_imag = self.MLP_out(y_imag)
        y = torch.stack([y_real, y_imag], dim=-1)
        y = F.softshrink(y, lambd=self.sparsity_threshold)
        y = torch.view_as_complex(y)
    ...
    def forward(self, x):
        ...
        if self.channel_independence == 1:
            ...
            x_noD = self.KAN_channel_WO_freq(x_noD, B, N, T)
        # [B, N, T, D]
