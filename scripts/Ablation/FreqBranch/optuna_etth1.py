...
def objective(trial, pred_len, logger):
    ...
    for line in result.stdout.splitlines():
        ...
        if mae is not None and rmse is not None:
            ...
            best_loss = max(best_loss, combined_loss)  # 或者使用 mae 作为损失
        # else:
...
def main():
    ...
    #     handlers=[logging.FileHandler('./optuna_log/ETTh1.log', mode='w'),  # 输出到文件
    ...
    for pred_len in pred_lens:
        # 配置日志记录器
        log_dir = './optuna_log/FreqBranch'
        os.makedirs(log_dir, exist_ok=True)
        log_file = f'{log_dir}/ETTh1_pred_len_{pred_len}.log'
        ...
        # 获取日志记录器
        logger = logging.getLogger(f'optuna_ETTh1_pred_len_{pred_len}')
        # logger.info(f"Optimizing for prediction length: {pred_len}")

        # 创建一个优化器实例
        # study = optuna.create_study(direction='minimize')  # 目标是最小化损失值
        # study = optuna.create_study(direction='minimize', study_name='ETTh1', storage='sqlite:///optunaDB/ETTh1.db', load_if_exists=True)
        study = optuna.create_study(
            direction='minimize',  # 目标是最小化损失值
            study_name=f'FreqBranch_ETTh1_{pred_len}',  # 每个pred_len单独的优化任务
            storage=f'sqlite:///optunaDB/FreqBranch_ETTh1_{pred_len}.db',  # 每个预测长度使用不同的数据库
            load_if_exists=True
        )

        logger.info(f"Optimizing for prediction length: {pred_len}")
        study.optimize(lambda trial: objective(trial, pred_len, logger), n_trials=5)  # 尝试10次训练（可以根据需要增加次数）
