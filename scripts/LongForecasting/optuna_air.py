...
def objective(trial, pred_len, logger):
    ...
    command = [
        ...
        "--model_id", f"air_{seq_len}_{pred_len}",
        "--model", model_name,
        "--data", "air",
        "--features", "M",
        "--seq_len", str(seq_len),
    ...
    for line in result.stdout.splitlines():
        ...
        if mae is not None and rmse is not None:
            ...
            # 选择最小的加权损失
            best_loss = min(best_loss, combined_loss)
...
def main():
    ...
    #     handlers=[logging.FileHandler('./optuna_log/air.log', mode='w'),  # 输出到文件
    ...
    for pred_len in pred_lens:
        ...
        log_file = f'{log_dir}/air_pred_len_{pred_len}.log'
        ...
        # 获取日志记录器
        logger = logging.getLogger(f'optuna_air_pred_len_{pred_len}')

        # logger.info(f"Optimizing for prediction length: {pred_len}")

        # 创建一个优化器实例
        # study = optuna.create_study(direction='minimize')  # 目标是最小化损失值
        # study = optuna.create_study(direction='minimize', study_name='air', storage='sqlite:///optunaDB/air.db', load_if_exists=True)
        study = optuna.create_study(
            direction='minimize',  # 目标是最小化损失值
            study_name=f'air_{pred_len}',  # 每个pred_len单独的优化任务
            storage=f'sqlite:///optunaDB/air_{pred_len}.db',  # 每个预测长度使用不同的数据库
            load_if_exists=True
        )
        logger.info(f"Optimizing for prediction length: {pred_len}")
