...
def objective(trial, pred_len, logger):
    ...
    command = [
        ...
        "--root_path", "./dataset/",
        "--data_path", "ETTm2.csv",
        "--model_id", f"ETTm2_{seq_len}_{pred_len}",
        "--model", model_name,
        "--data", "ETTm2",
        "--features", "M",
        "--seq_len", str(seq_len),
...
def main():
    ...
    #     handlers=[logging.FileHandler('./optuna_log/ETTm2.log', mode='w'),  # 输出到文件
    ...
    for pred_len in pred_lens:
        ...
        log_file = f'{log_dir}/ETTm2_pred_len_{pred_len}.log'
        ...
        # 获取日志记录器
        logger = logging.getLogger(f'optuna_ETTm2_pred_len_{pred_len}')
        # logger.info(f"Optimizing for prediction length: {pred_len}")

        # 创建一个优化器实例
        # study = optuna.create_study(direction='minimize')  # 目标是最小化损失值
        # study = optuna.create_study(direction='minimize', study_name='ETTm2', storage='sqlite:///optunaDB/ETTm2.db', load_if_exists=True)
        study = optuna.create_study(
            direction='minimize',  # 目标是最小化损失值
            study_name=f'ETTm2_{pred_len}',  # 每个pred_len单独的优化任务
            storage=f'sqlite:///optunaDB/ETTm2_{pred_len}.db',  # 每个预测长度使用不同的数据库
            load_if_exists=True
        )
